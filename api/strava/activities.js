const {getSession,freshSession,setSession,clearSession}=require('../../lib/strava.cjs');
module.exports=async(req,res)=>{
  ...
  try{
    ...
    try{session=await freshSession(session)}catch(error){
      if(error.status===400||error.status===401){clearSession(res);return reply(401,{connected:false,error:'reconnect_required'})}
      throw error;
    }
    ...
    for(let page=1;page<=10;page++){
      ...
      const r=await fetch(url,{headers:{Authorization:`Bearer ${session.access_token}`},signal:AbortSignal.timeout(12000)});
      if(r.status===401){clearSession(res);return reply(401,{connected:false,error:'reconnect_required'})}
  ...
  }catch(error){if(error.status===429)return reply(429,{error:'rate_limited'});if(error.name==='TimeoutError'||error.name==='AbortError')return reply(504,{error:'sync_timeout'});return reply(502,{error:'sync_failed'})}
};
