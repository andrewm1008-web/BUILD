const VERSION='0.4.0';
const IMPACT={long:1,marathon:.94,threshold:.86,interval:.80,easy:.36,strength:.28,recovery:.20,cross:.22};
...
let state={screen:profile.onboarded?'today':'onboarding',week:plan.currentWeek||5,selected:null,activity:null,onboardStep:0};
...
function save(){store.set('build.plan',plan);store.set('build.assessments',assessments);store.set('build.activities',activities);store.set('build.profile',profile)}
...
function weekStats(week){
  ...
  const distance=done.reduce((a,s)=>a+(assessed(s).actualDistanceKm||s.distanceKm||0),0);
...
function nextKey(){return plan.sessions.find(s=>s.week>=state.week && impact(s)>=.78 && !assessed(s)) || plan.sessions.find(s=>!assessed(s));}
...
function nav(){return `<nav class="bottom-nav">${[['today','⌂','Today'],['plan','▦','Plan'],['progress','◔','Progress'],['inbox','⇄','Activity']].map(([id,ico,label])=>`<button data-nav="${id}" class="${state.screen===id?'active':''}"><i>${ico}</i><span>${label}</span></button>`).join('')}</nav>`}
function header(){return `<header><button class="wordmark" data-nav="today">BUILD</button><button class="profile-btn" data-nav="settings">${escapeHTML((profile.name||'R')[0].toUpperCase())}</button></header>`}
...
function onboarding(){
  const steps=[
    ...
    `<div class="eyebrow">Your plan</div><h1>Bring the plan you trust.</h1><p>BUILD does not replace your coach or generate a new plan. Start with the example build or import a BUILD JSON plan later.</p><button class="primary" data-onboard-next>Use example plan</button>`,
    `<div class="eyebrow">Completed training</div><h1>Connect the work.</h1><p>Live Strava sync needs a secure backend. Until that is connected, BUILD can ingest your Strava activities CSV locally and match runs to the plan automatically.</p><button class="primary" data-onboard-finish>Enter BUILD</button>`
  ];
...
function settingsView(){return `${header()}<main><section class="page-head"><div class="eyebrow">Settings</div><h1>Your build.</h1></section><section class="card settings-list"><div><span>Race</span><b>${escapeHTML(plan.race)}</b></div><div><span>Goal</span><b>${escapeHTML(plan.goal)}</b></div><div><span>Activity source</span><b>${profile.source==='strava-csv'?'Strava CSV':'Demo / local'}</b></div><div><span>BUILD version</span><b>${VERSION}</b></div></section><button class="secondary full" id="resetData">Reset local data</button><p class="privacy">Your imported activity data stays in this browser in the current MVP.</p></main>${nav()}`}

function sessionSheet(s){
  const a=assessed(s), act=a?.activityId?activities.find(x=>x.id===a.activityId):null;
  return `<div class="sheet"><div class="panel"><div class="handle"></div><div class="eyebrow">${typeName(s.type)} · Impact ${Math.round(impact(s)*100)}</div><h2>${escapeHTML(s.title)}</h2><div class="info-block"><span>Planned target</span><p>${escapeHTML(s.target)}</p></div><div class="info-block"><span>Why it matters</span><p>${escapeHTML(s.objective)}</p></div>${act?`<div class="actual-block"><span>Matched activity</span><strong>${act.distanceKm.toFixed(1)} km · ${formatDuration(act.movingSeconds)}</strong><small>${escapeHTML(act.name)}</small></div>`:''}<div class="question"><span>How well did this session deliver its objective?</span><div class="score-grid">${[['nailed','Nailed it','Exactly the job'],['mostly','Mostly','Minor miss'],['modified','Modified','Different stimulus'],['missed','Missed','Didn’t happen']].map(([k,l,d])=>`<button data-rate="${k}" class="${a?.score===SCORES[k]?'selected':''}"><b>${l}</b><small>${d}</small></button>`).join('')}</div></div><button class="text-btn" data-close>Close</button></div></div>`;
}
function activitySheet(a){
  const candidates=rankMatches(a).slice(0,3);
  return `<div class="sheet"><div class="panel"><div class="handle"></div><div class="eyebrow">Imported activity</div><h2>${escapeHTML(a.name||'Run')}</h2><div class="actual-block"><strong>${a.distanceKm.toFixed(1)} km</strong><span>${formatDuration(a.movingSeconds)} · ${escapeHTML(a.date||'')}</span></div><div class="question"><span>Which planned session was this?</span><div class="match-list">${candidates.length?candidates.map(s=>`<button data-match="${encodeURIComponent(skey(s))}"><span><b>${escapeHTML(s.title)}</b><small>Week ${s.week} · ${escapeHTML(s.day)} · ${s.distanceKm||'—'} km</small></span><em>${matchScore(a,s)}%</em></button>`).join(''):'<p class="empty">No sensible planned match found.</p>'}</div></div><button class="text-btn" data-close>Close</button></div></div>`;
}
function render(){
...
function bind(){
  document.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>{state.screen=b.dataset.nav;state.selected=null;state.activity=null;render()});
  document.querySelectorAll('[data-week]').forEach(b=>b.onclick=()=>{state.week=+b.dataset.week;render()});
  document.querySelectorAll('[data-session]').forEach(b=>b.onclick=()=>{const k=decodeURIComponent(b.dataset.session);state.selected=plan.sessions.find(s=>skey(s)===k);render()});
  document.querySelectorAll('[data-rate]').forEach(b=>b.onclick=()=>{const old=assessed(state.selected)||{};assessments[skey(state.selected)]={...old,score:SCORES[b.dataset.rate],label:SCORE_LABEL[SCORES[b.dataset.rate]],at:new Date().toISOString()};save();state.selected=null;render()});
  document.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>{state.selected=null;state.activity=null;render()});
  ...
  document.querySelectorAll('[data-match]').forEach(b=>b.onclick=()=>{const s=plan.sessions.find(x=>skey(x)===decodeURIComponent(b.dataset.match));if(!s||!state.activity)return;assessments[skey(s)]={score:84,label:'Mostly',activityId:state.activity.id,actualDistanceKm:state.activity.distanceKm,at:new Date().toISOString(),source:'activity-match'};state.week=s.week;state.activity=null;state.selected=s;save();render()});
...
function estimatedSessionDate(s){if(!plan.startDate)return null;const d=new Date(`${plan.startDate}T12:00:00`);d.setDate(d.getDate()+(s.week-1)*7+((DAY_INDEX[s.day]??1)-d.getDay()));return d.toISOString().slice(0,10)}
...
function rankMatches(a){return plan.sessions.filter(s=>!assessed(s)||assessed(s).activityId===a.id).map(s=>[s,matchScore(a,s)]).filter(x=>x[1]>=45).sort((a,b)=>b[1]-a[1]).map(x=>x[0])}
function bestMatch(a){return rankMatches(a)[0]||null}

function parseCSV(text){
  const lines=[];let row=[],cur='',q=false;
  for(let i=0;i<text.length;i++){const c=text[i];if(c==='"'){if(q&&text[i+1]==='"'){cur+='"';i++}else q=!q}else if(c===','&&!q){row.push(cur);cur=''}else if((c==='\n'||c==='\r')&&!q){if(c==='\r'&&text[i+1]==='\n')i++;row.push(cur);if(row.some(x=>x.trim()))lines.push(row);row=[];cur=''}else cur+=c}row.push(cur);if(row.some(x=>x.trim()))lines.push(row);
  if(!lines.length)return[];const headers=lines[0].map(h=>h.trim());return lines.slice(1).map(r=>Object.fromEntries(headers.map((h,i)=>[h,r[i]??''])));
}
function findField(o,names){for(const n of names){let k=Object.keys(o).find(x=>x.toLowerCase()===n.toLowerCase());if(k&&o[k]!==undefined)return o[k]}return''}
function stravaRowsToActivities(rows){return rows.map((r,i)=>{const type=findField(r,['Activity Type','Type','Sport Type']);if(type&&!/run/i.test(type))return null;let dist=parseFloat(findField(r,['Distance','Distance.1']));if(!Number.isFinite(dist))return null;if(dist>1000)dist/=1000;let sec=parseFloat(findField(r,['Moving Time','Elapsed Time','Moving Time.1']))||0;let date=findField(r,['Activity Date','Date','Start Date']);let parsed=date?new Date(date):null;if(parsed&&!isNaN(parsed))date=parsed.toISOString().slice(0,10);else date='';return {id:String(findField(r,['Activity ID','Activity Id','ID'])||`csv-${Date.now()}-${i}`),name:findField(r,['Activity Name','Name'])||'Run',distanceKm:dist,movingSeconds:sec,date,source:'strava-csv'}}).filter(Boolean)}
...
document.getElementById('planFileInput').addEventListener('change',async e=>{let f=e.target.files[0];if(!f)return;try{let p=JSON.parse(await f.text());if(!Array.isArray(p.sessions)||!p.weeks)throw new Error();plan=p;assessments={};state.week=p.currentWeek||1;save();state.screen='plan';render()}catch{alert('That file is not a valid BUILD plan JSON.')}});
document.getElementById('activityFileInput').addEventListener('change',async e=>{let f=e.target.files[0];if(!f)return;try{const rows=parseCSV(await f.text()), imported=stravaRowsToActivities(rows);if(!imported.length)throw new Error();const byId=new Map(activities.map(a=>[a.id,a]));imported.forEach(a=>byId.set(a.id,a));activities=[...byId.values()];profile.source='strava-csv';save();render();alert(`${imported.length} running activities imported.`)}catch{alert('I could not read running activities from that CSV. Use the activities.csv file from your Strava export.')}});
